  ...
  app: {
    ...
    body: {
      button: {
        ...
        runDemo: '在线运行',
        fixDocs: '发现错误',
        fixDocTip: '如果該文檔存在錯誤或者不詳細，可以通過提交 PR 方式幫助完善文檔！',
        copyCode: '复制代码',
        showCode: '显示代码',
        showTsCode: '显示TS',
        showJsCode: '显示JS',
      ...
      other: {
        ...
        plan: {
          v1: 'v1.x 基於 vue2.6，支持所有主流的瀏覽器，實現表格的一切實用的功能',
          v2: 'v2.x 基於 vue2.6，支持所有主流的瀏覽器，同時兼具功能與效能',
          v3: 'v3.x 基於 vue2.6，支持現代瀏覽器並部分相容IE11，提升渲染效能',
          v4: 'v4.x 基於 vue3.2，只支持現代瀏覽器，不支持IE'
        },
        compatibility: '相容性變動',
        releases: '更新日誌',
        donation: '支持我们',
    ...
    api: {
      ...
      table: {
        desc: {
          id: '唯一標識（被某些特定的功能所依賴）',
          data: '表格數據（與 loadData 行爲一致，更新數據是不會重置狀態）',
          columns: '列配置',
          customs: '即將廢棄',
          height: '表格的高度；支持鋪滿父容器或者固定高度，如果設置 auto 爲鋪滿父容器（如果設置自適應時，必須確保存在父節點且不允許存在相鄰元素）',
          maxHeight: '表格的最大高度',
          ...
          showHeader: '是否显示表头',
          startIndex: '即將廢棄，請使用 seq-config.startIndex',
          ...
          cellStyle: '给单元格附加样式',
          headerCellStyle: '給表頭單元格附加樣式',
          footerCellStyle: '给表尾单元格附加样式',
          rowStyle: '給行附加樣式',
          headerRowStyle: '給表頭行附加樣式',
          footerRowStyle: '給表尾單元格附加樣式',
          showFooter: '是否顯示錶尾',
          footerData: '表尾數據',
          footerMethod: '表尾的数据获取方法，返回一个二维数组',
          mergeCells: '臨時合併指定的單元格 (不能用於展開行，不建議用於固定列、樹形結構)',
          mergeFooterItems: '臨時合併表尾 (不能用於展開行，不建議用於固定列、樹形結構)',
          spanMethod: '自定義合併函數，返回計算後的值 (不能用於虛擬滾動、展開行，不建議用於固定列、樹形結構)',
          footerSpanMethod: '表尾合併行或列，返回計算後的值 (不能用於虛擬滾動、展開行，不建議用於固定列、樹形結構)',
          showOverflow: '設置所有內容過長時顯示爲省略號（如果是固定列建議設置該值，提升渲染速度）',
          showHeaderOverflow: '設置表頭所有內容過長時顯示爲省略號',
          showAllOverflow: '即將廢棄，請使用 show-overflow',
          showHeaderAllOverflow: '即將廢棄，請使用 show-header-overflow',
          showFooterOverflow: '设置表尾所有内容过长时显示为省略号',
          sortMethod: '即將廢棄，請使用 sort-config.sortMethod',
          remoteSort: '即將廢棄，請使用 sort-config.remote',
          remoteFilter: '即將廢棄，請使用 filter-config.remote',
          columnWidth: '所有列寬度',
          columnMinWidth: '所有最小列寬度；會自動將剩餘空間按比例分配',
          columnKey: '是否需要爲每一列的 VNode 設置 key 屬性',
          ...
          selectChange: '即將廢棄，請使用 checkbox-change',
          checkboxChange: '只对 type=checkbox 有效，当手动勾选并且值发生改变时触发的事件',
          selectAll: '即將廢棄，請使用 checkbox-all',
          ...
          contextMenuClick: '即將廢棄，請使用 menu-click',
      ...
      tableColumn: {
        desc: {
          type: '列的类型（部分功能需要设置 column-config.useKey | row-config.useKey）',
          index: '即將廢棄，請使用 seq',
          seq: '序号',
          selection: '即將廢棄，請使用 checkbox',
          checkbox: '复选框',
          radio: '单选框',
          expand: '展开行',
          ...
          formatter: '格式化显示内容',
          indexMethod: '即將廢棄，請使用 seqMethod',
          ...
          sortMethod: '数据排序，只对 sortable 有效，列的排序方法，该方法的返回值用来决定该行的排序规则',
          remoteSort: '数据排序，是否使用服务端排序，如果设置为 true 则不会对数据进行处理',
          ...
          columnKey: '即將廢棄，請使用 table.column-key',
      ...
      grid: {
        desc: {
          layouts: '自定义布局',
          formConfig: '表单配置项',
          toolbar: '即將廢棄，請使用 toolbar-config',
          toolbarConfig: '工具栏配置',
          pagerConfig: '分页配置项',
          zoomConfig: '缩放配置项',
      ...
      toolbar: {
        desc: {
          ...
          print: '打印按钮配置',
          refresh: '刷新按钮配置',
          resizable: '即將廢棄，請使用 custom-config',
          setting: '即將廢棄，請使用 custom',
          custom: '自定义列配置'
        }
      },
      pager: {
