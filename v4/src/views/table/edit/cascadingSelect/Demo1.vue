...
import { ref } from 'vue'
import { VxeTableInstance, VxeColumnPropTypes, VxeTableEvents } from 'vxe-pc-ui'
...
// 格式化显示名称
const formatPanmeLabel: VxeColumnPropTypes.Formatter<RowVO> = ({ cellValue, row }) => {
  const ptype = row.attr3
  const cacheItem = cachePnameList.value.find(item => item.ptype === ptype)
...
// 更新级联选项列表
const updatePnameList = (row: RowVO) => {
  const ptype = row.attr3
  let pList: PTypeVO[] = []
  if (ptype) {
...
const ptypeChangeEvent = (row: RowVO) => {
  // 类型切换时更新级联的下拉数据
  row.attr4 = ''
  updatePnameList(row)
}
