<template>
  <div>
    <vxe-table
      ...
      <vxe-column field="date" title="标题溢出，显示为 tooltip 不换行不换行不换行不换行不换行不换行不换行不换行不换行" show-header-overflow show-overflow="title" show-footer-overflow></vxe-column>
      <vxe-column field="rate" title="Rate" show-header-overflow="title">
        <template #header>
          <span>标题显示原生 title ___________________________</span>
        </template>
      </vxe-column>
      <vxe-column field="address" title="换行换行换行换行换行换行换行换行换行换行换行换行换行换行" width="160"></vxe-column>
    </vxe-table>
  </div>
</template>
