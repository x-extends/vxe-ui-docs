<template>
  <div>
    <vxe-tree v-bind="treeOptions" v-on="gridEvents"></vxe-tree>
  </div>
</template>
...
const gridEvents: VxeTreeListeners<NodeVO> = {
  nodeDragstart ({ node }) {
    console.log(`拖拽开始 ${node.title}`)
  },
  nodeDragend ({ newNode, oldNode, dragPos }) {
    console.log(`拖拽完成，被拖拽行：${oldNode.title} 目标行：${newNode.title} 目标位置：${dragPos}`)
  }
}
</script>
