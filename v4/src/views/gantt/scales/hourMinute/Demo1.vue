<template>
  <div>
    <vxe-date-picker v-model="selectValue" type="datetime" value-format="yyyy-MM-dd HH" label-format="yyyy-MM-dd HH" @change="loadList()"></vxe-date-picker>
...
const loadList = () => {
  ...
  setTimeout(() => {
    ...
    for (let i = 0; i < 10; i++) {
      ...
      selectDate.setMinutes(XEUtils.random(0, 25))
      const startDate = XEUtils.toDateString(selectDate)
      selectDate.setMinutes(XEUtils.random(26, 59))
