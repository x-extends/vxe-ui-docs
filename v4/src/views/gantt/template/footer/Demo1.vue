<template>
  <div>
    <vxe-gantt v-bind="ganttOptions" :footerData="footerData">
      <template #title_footer="{ row }">
        <span style="color: red">平均进度：{{ row.progress }}%</span>
      </template>
    </vxe-gantt>
  </div>
</template>
...
const ganttOptions = reactive<VxeGanttProps<RowVO>>({
  ...
  columns: [
    ...
    { field: 'title', title: '任务名称', minWidth: 140, slots: { footer: 'title_footer' } },
...
const footerData = computed(() => {
  return [
    { seq: '合计', progress: meanNum(ganttOptions.data || [], 'progress') }
  ]
})
</script>
