<template>
  <div>
    <vxe-select v-model="val1" v-bind="selectOptions1"></vxe-select>
    <vxe-select v-model="val2" v-bind="selectOptions2"></vxe-select>
  </div>
</template>
...
const val1 = ref(null)
const selectOptions1 = reactive<VxeSelectProps>({
  width: 300,
  placeholder: '固定宽度 300px',
  options: [
...
const val2 = ref(null)
const selectOptions2 = reactive<VxeSelectProps>({
  width: '80%',
  placeholder: '百分比宽度 80%',
  options: [
    { value: 1001, label: 'table' },
    { value: 1002, label: 'grid' },
    { value: 1003, label: 'button' },
