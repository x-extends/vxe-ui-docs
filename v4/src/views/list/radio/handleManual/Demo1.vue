<template>
  <div>
    ...
    <vxe-list v-model:check-row-key="checkRowKey" v-bind="listOptions"></vxe-list>
  </div>
</template>
...
const checkRowKey = ref<VxeListPropTypes.CheckRowKey>()
...
const selectRadioEvent = () => {
  checkRowKey.value = '10011'
}

const clearRadioEvent = () => {
  checkRowKey.value = null
}

const getRadioEvent = () => {
  VxeUI.modal.alert(`${checkRowKey.value}`)
}
</script>
