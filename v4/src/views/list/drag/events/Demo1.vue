...
import { VxeListProps, VxeListListeners } from 'vxe-pc-ui'
...
const listEvents: VxeListListeners<RowVO> = {
  rowDragstart ({ row }) {
    console.log(`拖拽开始 ${row.id}`)
  },
  rowDragend ({ newRow, oldRow, dragPos }) {
    console.log(`拖拽完成，被拖拽行：${oldRow.id} 目标行：${newRow.id} 目标位置：${dragPos}`)
  }
}
</script>
