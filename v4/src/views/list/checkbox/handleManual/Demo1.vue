<template>
  <div>
    ...
    <vxe-list v-model:check-row-keys="checkRowKeys" v-bind="listOptions"></vxe-list>
  </div>
</template>
...
const selectCheckboxEvent = () => {
  checkRowKeys.value = ['10003', '10005']
}

const clearCheckboxEvent = () => {
  checkRowKeys.value = checkRowKeys.value.filter(val => val !== '10005')
}

const selectAllEvent = () => {
  checkRowKeys.value = listOptions.data.map(item => item.id)
}
...
const getCheckboxEvent = () => {
  VxeUI.modal.alert(checkRowKeys.value.length)
}
</script>
