<template>
  <div>
    <vxe-button @click="sortEvent('title', 'asc')">Title 升序</vxe-button>
    <vxe-button @click="sortEvent('title', 'desc')">Title 降序</vxe-button>
    <vxe-button @click="clearSortEvent">清除排序</vxe-button>

    <vxe-gantt ref="ganttRef" v-bind="ganttOptions"></vxe-gantt>
  </div>
</template>

<script lang="ts">
import Vue from 'vue'
import { VxeGanttInstance, VxeGanttProps } from 'vxe-gantt'
import { VxeTablePropTypes, VxeColumnPropTypes } from 'vxe-table'

interface RowVO {
  id: number
  title: string
  start: string
  end: string
  progress: number
}

export default Vue.extend({
  data () {
    const ganttOptions: VxeGanttProps<RowVO> = {
      border: true,
      height: 300,
      rowConfig: {
        isHover: true
      },
      taskBarConfig: {
        showProgress: true,
        showContent: true
      },
      taskViewConfig: {
        tableStyle: {
          width: 480
        }
      },
      columns: [
        { type: 'seq', width: 70 },
        { field: 'title', title: '任务名称', sortable: true },
        { field: 'start', title: '开始时间', width: 100, sortable: true },
        { field: 'end', title: '结束时间', width: 100, sortable: true }
      ],
      data: [
        { id: 10001, title: '任务R790', start: '2024-03-01', end: '2024-03-04', progress: 3 },
        { id: 10002, title: '任务C309', start: '2024-03-03', end: '2024-03-08', progress: 10 },
        { id: 10003, title: '任务E563', start: '2024-03-03', end: '2024-03-11', progress: 90 },
        { id: 10004, title: '任务P687', start: '2024-03-05', end: '2024-03-11', progress: 15 },
        { id: 10005, title: '任务L843', start: '2024-03-08', end: '2024-03-15', progress: 100 },
        { id: 10006, title: '任务B567', start: '2024-03-10', end: '2024-03-21', progress: 5 },
        { id: 10007, title: '任务V510', start: '2024-03-15', end: '2024-03-24', progress: 70 },
        { id: 10008, title: '任务G110', start: '2024-03-20', end: '2024-03-29', progress: 50 },
        { id: 10009, title: '任务I802', start: '2024-03-19', end: '2024-03-20', progress: 5 },
        { id: 10010, title: '任务K778', start: '2024-03-12', end: '2024-03-20', progress: 10 },
        { id: 10011, title: '任务E425', start: '2024-03-02', end: '2024-03-42', progress: 0 },
        { id: 10012, title: 'C计划', start: '2024-03-05', end: '2024-03-14', progress: 90 }
      ]
    }

    return {
      ganttOptions
    }
  },
  methods: {
    sortEvent (field: VxeColumnPropTypes.Field, order: VxeTablePropTypes.SortOrder) {
      const $gantt = this.$refs.ganttRef as VxeGanttInstance<RowVO>
      if ($gantt) {
        $gantt.sort({ field, order })
      }
    },
    clearSortEvent () {
      const $gantt = this.$refs.ganttRef as VxeGanttInstance<RowVO>
      if ($gantt) {
        $gantt.clearSort()
      }
    }
  }
})
</script>
