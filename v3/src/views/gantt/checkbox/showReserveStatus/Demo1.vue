<template>
  <div>
    ...
    <vxe-gantt ref="ganttRef" v-bind="ganttOptions" @page-change="pageChangeEvent"></vxe-gantt>
  </div>
</template>

<script lang="ts">
import Vue from 'vue'
import { VxeUI, VxeGanttInstance, VxeGanttProps } from 'vxe-gantt'
  ...
  methods: {
    ...
    handlePageData () {
      ...
      setTimeout(() => {
        ...
        this.ganttOptions.data = allList.slice((currentPage - 1) * pageSize, currentPage * pageSize)
    ...
    getSelectEvent () {
      ...
      if ($gantt) {
        ...
        const selectReserveRecords = $gantt.getCheckboxReserveRecords()
        VxeUI.modal.message({
