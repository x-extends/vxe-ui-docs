  ...
  <div>
    ...
    <vxe-tree ref="treeRef" v-bind="treeOptions">
      <template #extra="{ node }">
        ...
        <vxe-button mode="text" status="error" icon="vxe-icon-delete-fill" @click="removeEvent(node)">删除</vxe-button>
      </template>
    </vxe-tree>
  </div>
</template>
...
export default Vue.extend({
  ...
  methods: {
    ...
    insertNextNodeEvent (node: NodeVO) {
      const $tree = this.$refs.treeRef as VxeTreeInstance<NodeVO>
      if ($tree) {
        const record = {
          id: Date.now(),
    ...
    async insertChildEvent (node: NodeVO) {
      const $tree = this.$refs.treeRef as VxeTreeInstance<NodeVO>
      if ($tree) {
        const record = {
          id: Date.now(),
    ...
    getRemoveEvent () {
      const $tree = this.$refs.treeRef as VxeTreeInstance<NodeVO>
      if ($tree) {
        const removeRecords = $tree.getRemoveRecords()
        VxeUI.modal.message({
          content: `删除节点：${removeRecords.length}个`,
          status: 'success'
        })
