<template>
  <div>
    <div>
      ...
      <vxe-button status="primary" @click="clearCheckboxEvent">取消节点10005</vxe-button>
    ...
    <vxe-list :check-row-keys.sync="checkRowKeys" v-bind="listOptions"></vxe-list>
  </div>
</template>
...
export default Vue.extend({
  data () {
    ...
    const checkRowKeys: VxeListPropTypes.CheckRowKeys = []
  ...
  methods: {
    selectCheckboxEvent () {
      this.checkRowKeys = ['10003', '10005']
    },
    clearCheckboxEvent () {
      this.checkRowKeys = this.checkRowKeys.filter(val => val !== '10005')
    },
    selectAllEvent () {
      this.checkRowKeys = this.listOptions.data.map(item => item.id)
    },
    clearAllEvent () {
      this.checkRowKeys = []
    },
    getCheckboxEvent () {
      VxeUI.modal.alert(this.checkRowKeys.length)
    }
  }
})
</script>
